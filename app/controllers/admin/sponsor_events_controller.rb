...
module Admin
  class SponsorEventsController < Admin::ApplicationController
    ...
    private def sponsor_event_params
      params.require(:sponsor_event).permit(
        ...
        sp[:co_host_sponsorship_ids] = if params[:sponsor_event][:co_host_sponsorship_ids].present?
          params[:sponsor_event][:co_host_sponsorship_ids]
            .split(',')
            .map(&:strip)
            .reject(&:blank?)
            .map(&:to_i)
